...
def generate_inventor_questions(understanding: TechnicalUnderstandingResult, support_gaps: list[str] | None = None) -> list[InventorQuestion]:
    ...
    for parameter in understanding.parameters:
        if parameter.value is not None and not parameter.unit:
            ...
            if text not in existing:
                questions.append(InventorQuestion(question_id=f"Q-AUTO-{len(questions)+1:03d}", text=text, priority="P1", related_evidence_ids=parameter.evidence_ids)); existing.add(text)
    for equation in understanding.equations:
        if equation.status.value == "UNVERIFIED":
            ...
            if text not in existing:
                questions.append(InventorQuestion(question_id=f"Q-AUTO-{len(questions)+1:03d}", text=text, priority="P0", related_evidence_ids=equation.evidence_ids)); existing.add(text)
    for gap in support_gaps or []:
        text = f"独立权利要求支持存在缺口：{gap}，请补充材料或确认删除该特征。"
        if text not in existing:
            questions.append(InventorQuestion(question_id=f"Q-AUTO-{len(questions)+1:03d}", text=text, priority="P0")); existing.add(text)
